	...
	return (
			<div className="bg-slate-100 w-screen min-h-screen pt-16">
				...
				<div className="ml-60 p-6">
					<h1 className="text-2xl font-bold mb-4 text-slate-800 px-4">View Offers</h1>
					...
					<div className="p-4">
						{
							(toggleOffer ? incomingOffers : outgoingOffers).length > 0 ? (
								(toggleOffer ? incomingOffers : outgoingOffers).map((offer) => (
									<div key={offer.id} className="bg-white p-4 mb-4 rounded-lg shadow">
										<p className="text-slate-800"><strong>Offered To:</strong> {toTitleCase(offer.requested_item_name)}</p>
										<p className="text-slate-800"><strong>Offered Items:</strong> {toTitleCase(offer.offered_item_names.join(", "))}</p>
										<p className="text-slate-800"><strong>Message:</strong> {toTitleCase(offer.message)}</p>
										<p className="text-slate-800"><strong>Status:</strong> {toTitleCase(offer.status)}</p>
										{offer.status !== "pending" && (
											<p className="text-slate-800"><strong>Requested Item Location:</strong> {toTitleCase(offer.requested_item_location)}</p>
										)}
										<p className="text-slate-800"><strong>Created At:</strong> {new Date(offer.created_at).toLocaleDateString()}</p>
									</div>
								))
							) : (
								<p className="text-slate-800">No {toggleOffer ? "incoming" : "outgoing"} offers found.</p>
							)
						}
					</div>
