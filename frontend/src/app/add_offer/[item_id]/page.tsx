...
export default function AddOfferPage() {
			...
			const fetchItems = async () => {
				try {
					...
					if (requested_item_response.length === 1) {
						...
						throw new Error("Requested item not found or multiple item returned.")
					}
				} catch (error) {
	...
	const handleOfferSubmit = async () => {
		try{
			if(requestedItem && requestedItem.type === "free" && outgoingItems.length >0) {
			...
			else if(requestedItem && requestedItem.type === "exchange" && outgoingItems.length  === 0) {
								...
								{offerableItems.length > 0 ? (
									<ul className="list-disc pl-5">
									  {offerableItems.map((item) => {
									    const isSelected = outgoingItems.find((outItem) => outItem.id === item.id);
									    return (
									      <li
									        key={item.id}
									        ...
									        onClick={() => {
									          if (!isSelected) setOutgoingItems([...outgoingItems, item]);
														else setOutgoingItems(prev => prev.filter(outItem => outItem.id !== item.id));
									        }}
									      >
							...
							<div className="p-4">
								...
								(
									...
									<p className="text-slate-800 mb-4">
										Description: {toTitleCase(requestedItem.description)}<br />
										Condition: {toTitleCase(requestedItem.condition)}<br />
									...
									<ul className="list-disc pl-5">
										{outgoingItems.map((item) => (
											<li
												...
												onClick={() => {
													setOutgoingItems(prev => prev.filter(outItem => outItem.id !== item.id));
												}}
											>
												{toTitleCase(item.title)} - {toTitleCase(item.description)}
											</li>
